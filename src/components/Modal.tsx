...
const Modal:React.FC<Props> = ({ isOpen, onClose, center }) =>{
    ...
    const locations = JSON.parse(localStorage.getItem('locationHistory') || '[]').reverse();
            ...
            <div className="flex justify-center items-center h-full sm:pt-[500px] md:pt-0 xl:pt-0 lg:pt-0 xxl:pt-0 w-full">
                <div className='grid sm:place-items-center md:place-items-start xl:place-items-start lg:place-items-start xxl:place-items-start sm:grid-cols-1 md:grid-cols-3 xxl:grid-cols-3 lg:grid-cols-3 xl:grid-cols-3 gap-4'>
                    ...
                    <div className="w-full relative">
                        <input pattern="^(?:[^,\n]*,){3}[^,\n]*$"
                                 placeholder="Please enter a location in the format: street, postcode, town name, country" className="outline-0 p-3 w-full" value={searchText} onChange={handleInputChange}/>
                        <Link to={`/location/${locationSlug}`}>
                            <button onClick={(e)=>onClose(e)} className="absolute right-0 top-0 bg-black border-white border-2 p-3 duration-[0.1s] hover:border-blue-400 active:border-blue-400">
